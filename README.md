...
- **Crawler_seed module:** builds pagination for each site, previous import of a custom class that encapsulates the URL generation and parsing logic for a specific e-commerce site. Loading the desired URL end position from config.json, the module inserts the properly formatted URLs into the DB for the crawler module.
- **Crawler module:** loads the sequence of e-commerce search result pages generated by crawler_seed, visits them using Playwright and captures their fully rendered HTML. The data is then ready for product parsing and export.
...
- end_position: specifies how many pages will be generated by the crawler_seed module for future parsing. Note: the positions are indexed and run from 1 to (end_position - 1).
